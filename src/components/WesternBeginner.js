...
const WesternBeginner = ({setIsRecipeBeginner}) => {
    
    const [recipeBeginner, setRecipeBeginner] = useState([])
    
    useEffect(()=>{
        axios.get(`https://provision-backend.herokuapp.com/api/v1/cuisines/Western/recipes/beginner`)
        .then(response=>{
            ...
            <div className="mb-5">
                <nav className="d-flex justify-content-between mt-3">
                    <div className="display-4 ml-5">
                        Western Cuisine Recipes (Beginner) 
                    </div>
                    <div className="mr-5"> 
...
export default WesternBeginner;